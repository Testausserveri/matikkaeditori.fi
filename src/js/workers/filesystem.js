...
import uuid from "../worker-components/uuid.js"
    ...
    async init(){
        ...
        return new Promise(async (resolve, reject) => {
            try {
                 ...
                 */
                case 0: {
                    let index = await localStorage.getItem("matikkaeditori-index")
...
com.onMessage.addEventListener("message", async e => {
    ...
    case "init": {
        ...
        this_worker.filesystem_instances[e.id] = instance
        await instance.init()
        com.send("callback", { id: e.id, instance: instance.id, index: instance.index })
        break
    }
    case "read": {
        const instance = this_worker.filesystem_instances[e.content.instance]
